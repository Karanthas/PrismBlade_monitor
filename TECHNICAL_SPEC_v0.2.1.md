...
对于第三方或厂商官方 LUT，例如 Nikon 官方 N-Log 还原 LUT，v0.2.1 不应默认随仓库或发布包再分发，除非确认授权允许。开发环境可以支持本地可选 LUT 目录：App 启动时扫描 `PrismBlade/Resources/LUTs` 或 bundle 中约定文件名的 `.cube`；如果文件存在且解析成功，则将其作为可选内置/本地 LUT 展示；如果文件不存在、被 `.gitignore` 忽略、未打入 bundle 或解析失败，则静默隐藏该选项，不阻塞 App 启动，也不影响其他 LUT 导入流程。

本地可选厂商 LUT 的实现约束：

- `.cube` 文件不得作为必需 build resource；缺失时工程仍必须可构建、可运行。
- repository 中只保留 README、占位说明或自有授权资源；不可再分发的厂商 LUT 应通过 `.gitignore` 排除。
- `LUTStore` 应在运行时按文件存在性动态生成对应 descriptor，而不是在 `LUTDescriptor.builtIn` 中硬编码一个永远可见但可能无文件的选项。
- 运行时发现本地 Nikon N-Log 还原 LUT 时，可以显示为“Nikon N-Log to Rec.709（Local）”一类的本地可用项；未发现时不显示。
- 该 LUT 只适用于 N-Log 输入的监看还原，不应对 Rec.709 输入默认启用。阶段 4 可以先作为用户手动选择项；阶段 5 引入颜色空间解释后，再决定是否按 `FrameFormat.colorEncoding` 提供自动建议。
...
阶段 5 的首轮开发先使用程序生成素材闭环，不等待真实灰卡、色卡和肤色素材：

- XCTest 直接生成灰阶、中灰、clipping 和色块类输入，不把生成素材作为必需仓库文件。
- 离屏 Metal 测试使用小尺寸 float texture 模拟灰卡、灰阶 ramp、N-Log 编码中灰、HLG 参考点和高光阈值。
- 生成素材只用于验证 pass 顺序、shader 分支、阈值和 CPU/GPU 参考曲线一致性。
- 生成素材不能替代真实相机校准；它不覆盖 Nikon Z6III 传感器响应、白平衡、镜头、压缩、实际 N-Log/HLG 标记和现场光源影响。

后续收到真实素材后，阶段 5 需要进入深度验证：

- Nikon Z6III 拍摄的灰卡视频或 still frame，至少包含 Rec.709、N-Log、HLG 三种输入。
- Nikon Z6III 拍摄的色卡视频或 still frame，优先包含标准色卡、肤色块和高饱和红/绿/蓝。
- 欠曝、正常曝光、过曝高光、暗部噪声和肤色高光场景。
- 同一素材在 DaVinci Resolve、Final Cut 或 Nikon 官方工具中的参考画面。
- 同一素材的 waveform / RGB Parade 参考截图。
- 授权可用于本地开发的 Nikon N-Log -> Rec.709 `.cube` LUT，以及至少一个创意 LUT 和一个大尺寸 LUT。

真实素材到位前，阶段 5 的完成标准是“算法和渲染链路可测试、可切换、无占位 overlay”；真实素材到位后再校准 N-Log / HLG 主观观感、false color 映射偏好和 zebra 阈值习惯。
...
- 支持本地可选厂商 LUT：启动时仅在约定 `.cube` 文件实际存在且解析成功时展示，例如开发者本地放置的 Nikon N-Log 还原 LUT；缺失时隐藏，不作为仓库或发布包的必需资源。
...
- 新增 `ZebraPass`。
- 保留现有 UI 开关和斑马阈值设置。
- 首轮使用程序生成的灰阶、中灰、clipping 和色块输入完成自动化验证；真实灰卡、色卡和肤色素材到位后再做主观校准和阈值微调。
...
v0.2.1 采用单开发分支策略：所有阶段都直接在 `v0.2.1-metal-pipeline` 上按顺序开发和提交，不再为每个阶段创建短功能分支。
...
3. 阶段 1-7 都直接在 `v0.2.1-metal-pipeline` 上开发。
4. 每个阶段完成后提交阶段性 commit，并在提交说明中标明阶段，例如 `Stage 1: Add test target and fixtures`。
5. `v0.2.1-metal-pipeline` 完成全部验收后再合回 `main`。

### 提交规则

- 不在 `main` 上直接开发 v0.2.1。
- 每个阶段尽量拆成小提交，避免一个提交同时修改测试目标、帧源、Metal renderer、LUT 和 scope。
- 涉及 `project.pbxproj` 的改动应尽量集中在阶段 1 和阶段 3，减少 Xcode project 配置噪音。
- 每个阶段提交前至少完成：
  - Xcode build。
  - 当前阶段相关 XCTest。
  - v0.1.3 核心交互 smoke test。
- 如果某个阶段中途 App 暂时不可运行，应在同一阶段内尽快恢复，不把不可运行状态作为阶段完成点。
- 需要大规模重构时，优先通过连续小提交保留可回溯路径，而不是依赖额外分支隔离。

### 为什么这样设计

- `main` 保留稳定原型，避免 v0.2.1 架构迁移期间无法演示。
- 单开发分支降低分支管理成本，避免 Xcode project 文件在多分支之间反复冲突。
- 阶段仍按依赖顺序推进，保留清晰的实施边界和验收点。
- 测试目标先行，让后续 shader 和颜色转换有自动化 reference。
- Scope compute 放在后面，因为它依赖帧模型、Metal 显示、颜色转换和 GPU/CPU 同步策略。
- 对当前项目规模而言，阶段化提交比多分支并行更轻、更容易保持上下文连续。
