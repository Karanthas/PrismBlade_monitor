...
**PrismBlade** 是一个面向 Nikon Z6III 工作流的 iOS 相机监看 App 原型。当前分支正在按 `v0.2.1` Metal-first 计划推进。阶段 3 已完成：项目现在有真实 `CVPixelBuffer` 媒体帧模型、BGRA 模拟帧源、`AVAssetReader` 视频文件帧源，以及 `CVPixelBuffer -> MTLTexture -> MTKView drawable` 的 Metal 预览最小闭环。

> 英文主版本：[README.md](README.md)

## 当前状态

当前仓库包含一个可在 iOS Simulator 构建的 SwiftUI 原型工程，并完成了 `v0.2.1` 阶段 3 的 Metal 预览最小闭环。它目前**不会连接真实 Nikon 相机**，**不会实现 USB/PTP 通信**，也**不会移植 `libgphoto2`**。
...
- Metal 3D LUT texture 和 shader 采样。
- 基于真实像素的伪色和斑马纹处理。
...
Tests
  ...
  -> CubeFixtureFactory
  -> CPUReference
  -> 帧源、Metal bridge、LUT 和相机领域规则的 XCTest
```
...
阶段 3 的关键变化是主预览从 SwiftUI 合成画面切到 `MTKView`。你要验证的效果是：测试 target 能构建，fixtures 能生成，模拟帧源、视频文件帧源和 Metal texture bridge 自动化测试能全部通过。
...
当前阶段 3 测试共有 32 个。成功时终端会逐个打印测试用例，并以 `TEST EXECUTE SUCCEEDED` 结束。
...
阶段 3 开始，主预览由 `MTKView` 绘制：`VideoFrame.pixelBuffer` 会通过 `CVMetalTextureCache` 桥接为 `MTLTexture`，再由 `PreviewShaders.metal` 原样采样到 drawable。当前 LUT、伪色、斑马纹和 scope 仍是后续 pass，不会在阶段 3 改变 Metal 输出像素。
...
当前 LUT 导入会真实解析和校验，但还没有接入 Metal 3D texture 采样。下一步应在渲染管线中实现真正的 LUT pass 和强度混合。
...
1. 进入 `v0.2.1` 阶段 4：将 LUT 应用到真实像素。
