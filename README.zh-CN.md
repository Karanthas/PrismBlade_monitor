...
**PrismBlade** 是一个面向 Nikon Z6III 工作流的 iOS 相机监看 App 原型。当前分支正在按 `v0.2.1` Metal-first 计划推进。阶段 1 已完成：项目现在有 XCTest target、可生成的图像/LUT fixtures、CPU reference helper，以及对现有 LUT parser 和 Mock 相机领域规则的自动化保护。

> 英文主版本：[README.md](README.md)

## 当前状态

当前仓库包含一个可在 iOS Simulator 构建的 SwiftUI 原型工程，并加入了 `v0.2.1` 第一阶段测试基础。它目前**不会连接真实 Nikon 相机**，**不会实现 USB/PTP 通信**，也**不会移植 `libgphoto2`**。
...
- SwiftUI 负责布局和状态展示；当前画面仍是合成预览，不是最终 Metal 渲染管线。
...
阶段 1 不会新增可见的 App 功能。你要验证的效果是：新的测试 target 能构建，fixtures 能生成，自动化测试能全部通过。
...
当前阶段 1 测试共有 27 个。成功时终端会逐个打印测试用例，并以 `TEST EXECUTE SUCCEEDED` 结束。
...
1. 进入 `v0.2.1` 阶段 2：媒体帧模型与帧源。
   - 让 `VideoFrame` 逐步携带真实 `CVPixelBuffer`。
   - 如迁移期间仍需要 `phase`，只把它保留为过渡性的模拟状态。
   - 继续使用阶段 1 的 pixel buffer fixtures 作为测试输入。

2. 进入 `v0.2.1` 阶段 3：Metal 预览最小闭环。
   - 引入 `FrameProcessor`。
