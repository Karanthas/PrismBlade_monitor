...
**PrismBlade** 是一个面向 Nikon Z6III 工作流的 iOS 相机监看 App 原型。当前版本为 `v0.1.2`，核心目标是在 Xcode Simulator 中跑通监看体验、图像辅助工具、LUT 导入流程、底部相机控制、曝光模式限制规则和 Mock 相机通信边界。
...
`v0.1.2` 中 scope 面板已经缩小为约 40% 屏幕宽度，避免 waveform / RGB Parade 大面积遮挡监看画面。
...
被锁定的参数会置灰。点击时只显示原因，不打开滑块。相同规则会在 `MonitorSession`、`CameraCommandService` 和 `MockCameraTransport` 中重复校验。
