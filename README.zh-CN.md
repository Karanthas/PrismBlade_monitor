...
**PrismBlade** 是一个面向 Nikon Z6III 工作流的 iOS 相机监看 App 原型。当前分支正在按 `v0.2.1` Metal-first 计划推进。阶段 3 已完成，阶段 4 的首个 LUT 切片也已经接入：项目现在有真实 `CVPixelBuffer` 媒体帧模型、BGRA 模拟帧源、`AVAssetReader` 视频文件帧源、`CVPixelBuffer -> MTLTexture -> MTKView drawable` 的 Metal 预览闭环，以及在 Metal preview shader 中进行显示链路 3D LUT 采样的能力。

> 英文主版本：[README.md](README.md)

## 当前状态

当前仓库包含一个可在 iOS Simulator 构建的 SwiftUI 原型工程，并完成了 `v0.2.1` 阶段 3 的 Metal 预览闭环和阶段 4 的首个 LUT 渲染路径。它目前**不会连接真实 Nikon 相机**，**不会实现 USB/PTP 通信**，也**不会移植 `libgphoto2`**。
...
- Metal preview shader 中的显示链路 3D LUT 应用，并通过 LUT intensity 混合原始画面和 LUT 后画面。
...
- `v0.2.1` 阶段 1-4 XCTest target：
  ...
  - `MetalLUTShaderTests` 使用离屏渲染验证 LUT intensity 混合。
...
阶段 3 的关键变化是主预览从 SwiftUI 合成画面切到 `MTKView`。阶段 4 的首个切片新增了真实 LUT texture 上传和 fragment shader LUT 采样。你要验证的效果是：测试 target 能构建，fixtures 能生成，模拟帧源、视频文件帧源、Metal texture bridge、LUT repository、LUT pass 和 Metal LUT shader 自动化测试能全部通过。
...
当前阶段 3-4 测试共有 39 个。成功时终端会逐个打印测试用例，并以 `TEST EXECUTE SUCCEEDED` 结束。
...
自动化测试不依赖这些真实素材；测试会临时生成小型 `.mov` fixture 来验证 `AVAssetReader`，并用模拟 BGRA pixel buffer 验证 Metal bridge。后续阶段 5/7 的颜色转换和真实素材手动验证会优先使用 `material/` 中的文件。
...
阶段 3 开始，主预览由 `MTKView` 绘制：`VideoFrame.pixelBuffer` 会通过 `CVMetalTextureCache` 桥接为 `MTLTexture`，再由 `PreviewShaders.metal` 采样到 drawable。当前阶段 4 的 LUT 切片还会绑定 3D LUT texture，并在 fragment shader 中按 intensity 混合 LUT 输出。伪色、斑马纹和 scope 仍是后续 pixel-processing pass，不会在当前阶段改变 Metal 输出像素。
...
iPhone 17 Simulator 上 app 构建、测试构建和测试执行均已成功。正常 `.metal` 编译路径依赖已安装的 Metal Toolchain；在受限 shell 里，Xcode 可能看不到 cryptex 挂载的 Metal Toolchain，或打印 CoreSimulator 日志权限警告。如果沙盒无法访问 Metal Toolchain、CoreSimulator 或 `testmanagerd`，请直接从 Terminal 或 Xcode 运行构建和测试。

## 下一步计划

推荐按以下顺序继续开发：

1. 继续打磨 `v0.2.1` 阶段 4 LUT 路径。
   - 使用可选本地 Nikon / N-Log LUT 做更多真实素材验证。
   - 除非确认授权允许再分发，否则继续把本地厂商 LUT 资源留在仓库之外。
   - 在阶段 5 决定是否根据 `FrameFormat.colorEncoding` 提供 LUT 自动建议。
